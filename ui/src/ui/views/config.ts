...
// Section definitions - labels are translated at render time
...
function getSectionLabel(key: string): string {
  const translationKey = `config.section.${key}` as any;
  try {
    const translated = t(translationKey);
    if (translated && !translated.startsWith("[")) return translated;
  } catch {}
  // Fallback to original label from SECTIONS
  const section = SECTIONS.find(s => s.key === key);
  return section?.label ?? key.charAt(0).toUpperCase() + key.slice(1);
}
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          ...
          <span class="pill pill--sm ${validity === "valid" ? "pill--ok" : validity === "invalid" ? "pill--danger" : ""}">${validity === "valid" ? t("config.valid") : validity === "invalid" ? t("config.invalid") : t("config.unknown")}</span>
        </div>
      ...
      <main class="config-main">
        ...
        <div class="config-actions">
          <div class="config-actions__left">
            ${hasChanges ? html`
              <span class="config-changes-badge">${props.formMode === "raw" ? t("config.unsavedChanges") : `${diff.length} ${diff.length !== 1 ? t("config.unsavedChangesPlural") : t("config.unsavedChange")}`}</span>
            ` : html`
        ...
        ${hasChanges && props.formMode === "form" ? html`
          <details class="config-diff">
            <summary class="config-diff__summary">
              <span>${diff.length !== 1 ? t("config.viewPendingChanges", { count: String(diff.length) }) : t("config.viewPendingChange")}</span>
